---
title: SECS/GEM 从零到一（一）：用 C# 实现 HSMS 通讯，附开源库
tags: [SECS/GEM, HSMS, 半导体, EAP, C#, 设备自动化, GEM300]
categories: [半导体自动化]
---
...
本系列将用大白话 + 可运行代码，把 SECS/GEM 从通讯层到 GEM300 完整讲一遍。本文为第一篇：**HSMS 通讯层**。

## 一、SECS/GEM 分层模型

很多人一上来就懵，是因为把一堆标准混在一起。按职责分就三层：
...
现在工厂基本都用 **HSMS**（TCP/IP），SECS-I 串口已经很少见。所以从 HSMS 入手最实际。
...
本文讲完了 HSMS 通讯层。下一篇写 **SECS-II 消息与 SML**：S1F1/S1F2 设备识别、S2F33 数据变量定义、S6F11 事件上报，附完整 C# 实现。

配套开源库 [SecsGem.Net](https://github.com/MC-xiaohe/SecsGem.Net) 正在开发中（C#，对标商业 SDK，含全中文文档与 GEM300 E87/E90 示例），欢迎 Star 和提 Issue。有 SECS/GEM 联机调试、GEM300 集成需求的同行欢迎交流（作者提供定制开发与远程排障服务）。
